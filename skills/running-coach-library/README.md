...
支持 SKILL.md 的客户端（如 KAI，走未认证 GitHub API、不带 token，故本仓库为 **public**）从本仓库安装：
技能位于子目录 `skills/running-coach-library/`，**必须带该子目录路径**才会把 `references/`、`scripts/` 一起装进沙盒。

```
ly-ly-666/running-coach-skill/skills/running-coach-library
```

或完整 URL：`https://github.com/ly-ly-666/running-coach-skill/tree/main/skills/running-coach-library`

装好后用斜杠命令触发（id 取自 SKILL.md frontmatter 的 `name`）：

```
/running-coach-library 给一个针对 10 公里比赛的中周期训练安排
```

结构符合 Anthropic Skill 标准：`SKILL.md` 为入口，`references/` 为本体。
...
| `hansons.md` | 汉森马拉松训练法 | 累积疲劳模型、3 quality day、长限机会窗口≤32km、减量 | 🟢 OCR 提取 |
| `nsca.md` | NSCA 力量训练指南 | 目标→负荷参数表、训练课编排、周期化、专项行动进阶 | 🟢 OCR 提取 |
...
- **2 本（hansons/nsca）**：原始 PDF 为扫描版（无文本层），已用 **tesseract(chi_sim) 逐页 OCR**
  后整理成方法稿。OCR 可能有少量错字，但各篇**关键配速/距离/负荷数字已逐处人工核对**，文首注明来源限制。
